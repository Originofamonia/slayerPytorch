...
def compute_loss(inputs: torch.Tensor, target: torch.Tensor, time_start_idx: int = None):
    """
    :param inputs: tensor of shape (batch, 3, time)
    ...
    if time_start_idx:
        inputs = inputs[..., time_start_idx:]
        target = target[..., time_start_idx:]

    return torch.mean(torch.sqrt(torch.sum((inputs - target) ** 2, dim=1)))
...
def main():
    ...
    trainingSet = SdDataset(datasetPath=netParams['training']['path']['in'],
                            sampleFile=netParams['training']['path']['train'],
                            samplingTime=netParams['simulation']['Ts'],
                            sampleLength=netParams['simulation']['tSample'])
    trainLoader = DataLoader(dataset=trainingSet, batch_size=12, shuffle=False, num_workers=4)

    testingSet = SdDataset(datasetPath=netParams['training']['path']['in'],
