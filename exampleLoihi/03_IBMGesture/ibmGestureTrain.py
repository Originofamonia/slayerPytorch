...
def main():
    ...
    # Visualize the inputs spikes (first five samples).
    for i in range(5):
        inputs, target, label = trainingSet[i]
        snn.io.showTD(snn.io.spikeArrayToEvent(inputs.reshape((2, 128, 128, -1)).cpu().data.numpy()))
    ...
    for epoch in range(5):
        ...
        # Training loop.
        for i, (inputs, target, label) in enumerate(trainLoader, 0):
            net.train()

            # Move the inputs and target to correct GPU.
        ...
        for i, (inputs, target, label) in enumerate(testLoader, 0):
            net.eval()
            with torch.no_grad():
                inputs = inputs.to(device)
                target = target.to(device)
